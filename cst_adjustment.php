...
if($_GET['action'] == 'save'){
	if(($_POST[request] == 'Services') || ($_POST[request] == 'Debit Note')||($_POST[request] == 'Refund')||($_POST[request] == 'Prorate Debit Note')){
		...
		if($_POST[request] == 'Prorate Debit Note'){
			$_POST[approved_by] = 'The Prorator function';
			$_POST[request] = 'Debit Note';
		}
	}else{
		$_POST[adjustments] = abs($_POST[adjustments]);

		if($_POST[request] == 'Prorate Credit Note'){
			$_POST[approved_by] = 'The Prorator function';
			$_POST[request] = 'Credit Note';
		}
	}
	save_entry();
	
	if($_POST[request] == 'Refund'){
		if(substr($_POST[product],0,17) == 'Equipment Deposit'){
		...
		$_POST[reciept] += 1;
		$_POST[details] = "Credit Note for earlier charge Matching Refund [".$_POST[details]."]";
		if($_POST[product] != 'Payment'){ //ie A pament refund
			save_entry();
		}
	}
...
$query_cst_details = "
	...
	FROM 
		...
		LEFT OUTER JOIN ps_products ON (accounts_cstm.download_bandwidth_c=ps_products.name)
	WHERE 
		accounts.deleted = '0' AND 
		cn_contracts.deleted = '0' AND 
		(ps_products.deleted = '0' OR ps_products.deleted IS NULL) AND 
		accounts_cstm.crn_c = '$_GET[account_id]'
	";

//echo nl2br($query_cst_details);
...
$currency_query = "
	...
	from
		...
		inner join accounts on (accounts.id = accounts_cstm.id_c)
	WHERE
		accounts_cstm.mem_id_c = '".trim($row_cst_details[parent_id])."' AND
		accounts.deleted = 0
	LIMIT 1
";
...
if($currency_result[selected_billing_currency] != 'USD' and $currency_result[selected_billing_currency] != 'UGX'){
	$error = 'Account ['.$_GET[account_id].'] with parent ['.$row_cst_details[parent_id].'] has no Billing currency on its parent account .... ';
...
<!-- Hidding and showing the erroneous value text element -->
<script language="JavaScript" type="text/JavaScript">
function show_form_element(){
	if((document.getElementById('request').value == 'Credit Note')||(document.getElementById('request').value == 'Debit Note')) {
		document.getElementById('error_tr').style.display = '';
		document.getElementById('corrected_tr').style.display = '';
		document.getElementById('adjustments').value = '';
		document.getElementById('wrong_value').value = '';
		document.getElementById('wrong_value_submitted').value = '';
	}else{
		document.getElementById('error_tr').style.display = 'none';
		document.getElementById('corrected_tr').style.display = 'none';
	}
}
...
function show_correction(){
	...
	var wrong_value = document.getElementById('wrong_value').value;

	if(document.getElementById('adjustments').value == '') {
		document.getElementById('correct_value').value = 'missing data';
	}else if((document.getElementById('wrong_value').value == '')&&(parseFloat(document.getElementById('adjustments').value) > '0')){		
		if(document.getElementById('request').value == 'Credit Note'){
			wrong_value = Math.abs(parseFloat(adjustments));
			document.getElementById('wrong_value').value = wrong_value;

			document.getElementById('wrong_value_submitted').value = -Math.abs(parseFloat(wrong_value));
			document.getElementById('correct_value').value = (parseFloat(adjustments) + -Math.abs(parseFloat(wrong_value)));
		}else if(document.getElementById('request').value == 'Debit Note'){
			wrong_value = '0';
			document.getElementById('wrong_value').value = wrong_value;
...
<form id="form1" name="form1" method="post" action="cst_adjustment.php?action=save">
  <table width="100%" border="0" align="left" cellpadding="2" cellspacing="1">
   ...
   <tr>
      <td width="16%" valign="top" bgcolor="#CCCCCC" class="style4">Set Adjustment Date</td>
      <td width="84%" valign="middle"><input name="entry_date" class="style1" id="entry_date" value="<? echo date('Y-m-d'); ?>" wdg:mondayfirst="false" wdg:subtype="Calendar" wdg:mask="<?php echo $KT_screen_date_format; ?>" wdg:type="widget" wdg:singleclick="false" wdg:restricttomask="no" wdg:readonly="true" /></td>
    </tr>
    <tr>
      <td width="16%" valign="top" bgcolor="#CCCCCC" class="style4">Invoice Number </td>
      <td>
      	<span class="style4">
      		<input name="matched_invoice" type="text" class="style1" id="matched_invoice" />
      	</span></td>
    </tr>
    <tr>
      <td width="16%" valign="top" bgcolor="#CCCCCC" class="style4">Adjustment Type</td>
      <td width="84%" valign="top"><label>
        <select name="request" class="style1" id="request" onchange="javascript:show_form_element();">
          ...
          <option value="Prorate Debit Note">PRORATED - Debit Note</option>
          <option value="Prorate Credit Note">PRORATED - Credit Note</option>
          <option value="Refund">Payment Refund</option>
          <option value="Services">Corrective service billing</option>
          <option value="Services">Termination service billing</option>
          </select></label>
        <? echo display_products_dropdown(''); ?>
      </td>
    </tr>   
    <tr>
      <td width="16%" valign="top" bgcolor="#CCCCCC" class="style4">Adjustment Value</td>
      <td>
      	<span class="style4">
      		<input name="adjustments" type="text" class="style1" id="adjustments" onchange="javascript:show_correction();" />
      	</span></td>
    </tr>
    <tr id="error_tr" style="display:none;">
      <td width="16%" valign="top" bgcolor="#CCCCCC" class="style4">Value before Adjustment</td>
      <td>
      	<span class="style4">
      		<input name="wrong_value" type="text" class="style1" id="wrong_value" onchange="javascript:show_correction();"/>
            <input name="wrong_value_submitted" type="hidden" class="style1" id="wrong_value_submitted" value=""/>
      	</span></td>
    </tr>
    <tr id="corrected_tr" style="display:none;">
      <td width="16%" valign="top" bgcolor="#CCCCCC" class="style4"> Value after Adjustment</td>
      <td>
      	<span class="style4">
      		<input name="correct_value" type="text" class="style1" id="correct_value" value="" readonly="readonly" />
      	</span></td>
    </tr>
    <tr>
      <td width="16%" valign="top" bgcolor="#CCCCCC" class="style4">Currency Choice</td>
      <td><select name="currency" class="style1" id="currency">
        ...
        <span class="style1">Using the following dollar rate : <?php $ret_set = get_rate(date('Y-m-d')); echo number_format($ret_set[rate],0)." set on date : ".$ret_set[rate_date]; ?> </span></td>
    </tr>
    <tr>
      <td width="16%" valign="top" bgcolor="#CCCCCC" class="style4">Being Adjustment for</td>
      <td><label>
        <input name="details" type="text" class="style1" id="details" size="60" />
      </label></td>
    </tr>
    <tr>
      <td width="16%" valign="top" bgcolor="#CCCCCC" class="style4">Approved by</td>
      <td><label>
