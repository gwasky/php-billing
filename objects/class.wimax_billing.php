...
/*
	...
	`deleted` TINYINT NOT NULL, PRIMARY KEY  (`id`)) ENGINE=MyISAM;
*/
...
class wimax_invoicing extends POG_Base {
	...
	*/
	...
	function wimax_invoicing($generation_date='', $parent_id='', $billing_date='', $previous_balance='', $payments_sum='', $adjustments_sum='', $charges_sum='', $amount_payable='', $period='', $details='', $invoice_number='', $deleted=0)
	{
	...
	*/
	function GetList($fcv_array = array(), $sortBy='', $ascending=true, $limit='')
	{
		...
		while ($row = Database::Read($cursor))
		{
			...
			$wimax_invoicing->deleted = $this->Unescape($row['deleted']);
	...
	*/
	function Save()
	{
		...
		if ($rows > 0)
		{
			...
			`invoice_number`='".$this->Escape($this->invoice_number)."', 
			`deleted`='".$this->Escape($this->deleted)."' where `id`='".$this->id."'";
		}
		else
		{
			$this->pog_query = "insert into `wimax_invoicing` (`generation_date`, `parent_id`, `billing_date`, `previous_balance`, `payments_sum`, `adjustments_sum`, `charges_sum`, `amount_payable`, `period`, `details`, `invoice_number`, `deleted` ) values (
