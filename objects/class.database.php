...
{	
	...
	public static function mysqli_escape($text){
		$databaseName = $GLOBALS['configuration']['db'];
		$serverName = $GLOBALS['configuration']['host'];
		$databaseUser = $GLOBALS['configuration']['user'];
		$databasePassword = $GLOBALS['configuration']['pass'];
		$databasePort = $GLOBALS['configuration']['port'];
		$mysqli_connection = mysqli_connect($serverName, $databaseUser, $databasePassword, $databaseName);
		
		return mysqli_real_escape_string($mysqli_connection, $text);
	}
}
?>
