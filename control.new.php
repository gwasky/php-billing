...
//ps_products.type = 'Service'
	...
	foreach($parent_accts as $parent_id => $parent_acct){
		...
		if(	!(
				(round($invoicing->charges_sum,2) == 0.00) and
				(round($invoicing->adjustments_sum,2) == 0.00)
			) and (
				//ie for existing Proforma invoices
				trim($invoicing->parent_id) != ''
			)
		   ){
			$checks = array(
			...
			if(count($check_objects) == 0){
				...
				if(trim($check_object->parent_id) != ''){
					...
					$html .= "
						Skipping save/update action on Invoice number ".$check_object->invoice_number."<br>
						++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
						".display_emailinvoice_byid($check_object->id)."<br>
						++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
					";
				}
				while(count($check_objects[++$i]) != 0){
					$invoice = $check_objects[$i];
					if(trim($invoice->parent_id) != '' ) { $invoice->Delete(); }
				}
			}
		}else{
			if(trim($invoicing->parent_id) != ''){
				$html .= "Zero charged Inv:-> Acct no: ".$invoicing->parent_id." : Bal ".$invoicing->previous_balance." Adjustments ".$invoicing->adjustments_sum." Charges ".$invoicing->charges_sum." Amount Payable ".$invoicing->amount_payable."<br>";
			}else{
				$html .= "Shady invoice type  <br>
				++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
				".nl2br(print_r($invoicing, TRUE))."<br>
				++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
				";
			}
		}
