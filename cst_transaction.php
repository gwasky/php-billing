...
//ACCESS DEFINITIONS MOVED TO THE IFS SECTION NOT HERE
//REQUEST NOT TO HAVE CREDIT COLLECTORS ENTER PAYMENTS.
//$MM_authorizedUsers = "Customer Care - Operations,Super,CC Sales";

//BY DEFAULT TELL THE SYSTEM TO DO ACCESS CHECK
...
function isAuthorized($strUsers, $strGroups, $UserName, $UserGroup) {
  ...
  if (!empty($UserName)) { 
    ...
    if (in_array(trim($UserName), $arrUsers)) { 
      $isValid = true; 
    }else{
		//echo "User ".$UserName." is not in list ".print_r($arrUsers,true)."<hr>";
	}
    // Or, you may restrict access to only certain users based on their username. 
    if (in_array(trim($UserGroup), $arrGroups)) { 
      $isValid = true; 
    }else{
...
if($_GET['action'] == 'save'){
	...
	if(preg_replace("/[^a-zA-Z0-9]+/","",trim($_POST[payment_type])) == '' or preg_replace("/[^a-zA-Z0-9]+/","",trim($_POST[payment_details])) == ''){
		echo '
			Please Enter a payment Mode and the reason for the payment under "Being payment for"<hr>
			<br>
			<a href="cst_transaction.php?account_id='.$_POST[account_id].'">Try entering payment again.</a>
		';
	}else{
		save_entry();
	...
	//ACCESS CHECK
	$MM_authorizedUsers = "Finance Credit and Control,Finance - Collections,Customer Care - Operations,Super,CC Sales,Bill Delivery";
	if (!((isset($_SESSION['MM_Username'])) && (isAuthorized("",$MM_authorizedUsers, $_SESSION['MM_Username'], $_SESSION['MM_UserGroup'])))) {   
		$MM_qsChar = "?";
		$MM_referrer = $_SERVER['PHP_SELF'];
		if (strpos($MM_restrictGoTo, "?")) $MM_qsChar = "&";
...
<form id="form1" name="form1" method="post" action="cst_transaction.php?action=save">
  <table width="100%" border="0" align="left" cellpadding="2" cellspacing="1">
   ...
   <tr>
      ...
      <td width="47%" valign="middle"><input name="entry_date" class="style1" id="entry_date" value="<?php echo date('Y-m-d'); ?>" wdg:mondayfirst="false" wdg:subtype="Calendar" wdg:mask="<?php echo $KT_screen_date_format; ?>" wdg:type="widget" wdg:singleclick="false" wdg:restricttomask="no" wdg:readonly="true" /></td>
    ...
    <tr>
      ...
      <td width="47%" valign="top"><label>
        <select name="payment_type" class="style1" id="payment_type">
          ...
          <option value="Bounced cheque">Bounced Cheque Reversal</option>
          <option value="Staff Recovery">Off Staff Salary</option>
          <option value="Benefits">Benefits/entitled for staff</option>
          <option value="Company Benefits">Benefits/entitled for companies</option>
          <option value="Warid - Vendor offset">Warid - Vendor payment offset</option>
    ...
    <tr>
      ...
      <td><span class="style1">Cheque/Transfer Number
          <label>
